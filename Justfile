...
release-binary:
    ...
    cargo run -p danni-server --bin generate_schema
...
release-windows:
    ...
    if [ "$(uname)" = "Darwin" ] || [ "$(uname)" = "Linux" ]; then
        ...
        docker volume create danni-windows-cache || true
        docker run --rm \
            -v "$(pwd)":/usr/src/myapp \
            -v danni-windows-cache:/usr/local/cargo/registry \
            -w /usr/src/myapp \
            rust:latest \
            sh -c "{{win_docker_build_sh}}"
    else
        echo "Building Windows executable using Docker through PowerShell..."
        powershell.exe -Command "docker volume create danni-windows-cache; \`
            docker run --rm \`
                -v ${PWD}:/usr/src/myapp \`
                -v danni-windows-cache:/usr/local/cargo/registry \`
                -w /usr/src/myapp \`
...
copy-binary BUILD_MODE="release":
    @if [ -f ./target/{{BUILD_MODE}}/dannid ]; then \
        echo "Copying dannid binary from target/{{BUILD_MODE}}..."; \
        cp -p ./target/{{BUILD_MODE}}/dannid ./ui/desktop/src/bin/; \
    else \
...
copy-binary-intel:
    @if [ -f ./target/x86_64-apple-darwin/release/dannid ]; then \
        echo "Copying Intel dannid binary to ui/desktop/src/bin with permissions preserved..."; \
        cp -p ./target/x86_64-apple-darwin/release/dannid ./ui/desktop/src/bin/; \
    else \
...
copy-binary-windows:
    @powershell.exe -Command "if (Test-Path ./target/x86_64-pc-windows-gnu/release/dannid.exe) { \
        Write-Host 'Copying Windows binary and DLLs to ui/desktop/src/bin...'; \
        Copy-Item -Path './target/x86_64-pc-windows-gnu/release/dannid.exe' -Destination './ui/desktop/src/bin/' -Force; \
...
run-server:
    @echo "Running server..."
    cargo run -p danni-server --bin dannid agent
...
make-ui-windows:
    ...
    #!/usr/bin/env sh
    set -e
    if [ -f "./target/x86_64-pc-windows-gnu/release/dannid.exe" ]; then \
        echo "Cleaning destination directory..." && \
        rm -rf ./ui/desktop/src/bin && \
        mkdir -p ./ui/desktop/src/bin && \
        echo "Copying Windows binary and DLLs..." && \
        cp -f ./target/x86_64-pc-windows-gnu/release/dannid.exe ./ui/desktop/src/bin/ && \
...
win-bld profile allparam:
  cargo run {{profile}} -p danni-server --bin  generate_schema
