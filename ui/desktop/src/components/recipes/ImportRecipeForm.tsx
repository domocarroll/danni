...
const importRecipeSchema = z
  .object({
    deeplink: z
      .string()
      .refine(
        (value) => !value || value.trim().startsWith('danni://recipe?config='),
        'Invalid deeplink format. Expected: danni://recipe?config=...'
      ),
    recipeUploadFile: z
      .instanceof(File)
      .nullable()
...
export default function ImportRecipeForm({ isOpen, onClose, onSuccess }: ImportRecipeFormProps) {
  ...
  const parseDeeplink = async (deeplink: string): Promise<Recipe | null> => {
    try {
      ...
      if (!cleanLink.startsWith('danni://recipe?config=')) {
        throw new Error('Invalid deeplink format. Expected: danni://recipe?config=...');
      }

      const recipeEncoded = cleanLink.replace('danni://recipe?config=', '');
                        ...
                        return (
                          <div className={isDisabled ? 'opacity-50' : ''}>
                            ...
                            <textarea
                              ...
                              placeholder="Paste your danni://recipe?config=... deeplink here"
                              rows={3}
                            ...
                            >
                              Paste a recipe deeplink starting with "danni://recipe?config="
                            </p>
                            {field.state.meta.errors.length > 0 && (
