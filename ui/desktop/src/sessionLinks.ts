 ...
 * @param url The deep link URL (danni://sessions/:shareToken)
...
export async function openSharedSessionFromDeepLink(
  ...
  try {
    if (!url.startsWith('danni://sessions/')) {
      throw new Error('Invalid URL: URL must use the danni://sessions/ scheme');
    }

    // Extract the share token from the URL
    const shareToken: string = url.replace('danni://sessions/', '');
    ...
    setView('sharedSession', {
      ...
      shareToken: url.replace('danni://sessions/', ''),
      baseUrl,
    });
