...
if (MAIN_WINDOW_VITE_DEV_SERVER_URL) {
  ...
  console.log('[Main] Development mode: Forcing protocol registration for danni://');
...
if (process.platform !== 'darwin') {
  ...
  if (!gotTheLock) {
    ...
    app.on('second-instance', (_event, commandLine) => {
      const protocolUrl = commandLine.find((arg) => arg.startsWith('danni://'));
      if (protocolUrl) {
  ...
  const protocolUrl = process.argv.find((arg) => arg.startsWith('danni://'));
  if (protocolUrl) {
    app.whenReady().then(() => {
      handleProtocolUrl(protocolUrl);
    });
...
async function appMain() {
  ...
  // App menu
  const appMenu = menu?.items.find((item) => item.label === 'Danni');
  if (appMenu?.submenu) {
