...
impl DanniWiki {
    ...
    fn check_html_implementation(&self, content: &str) -> bool {
        ...
        let has_wiki_elements = content.contains("<h1") && // Has headings
                              ...
                              content.contains("Danni") && // Mentions Danni
...
#[async_trait]
impl Evaluation for DanniWiki {
    async fn run(
        &self,
        agent: &mut BenchAgent,
        _run_loc: &mut BenchmarkWorkDir,
    ) -> anyhow::Result<Vec<(String, EvalMetricValue)>> {
        println!("DanniWiki - run");

        // Collect baseline metrics (execution time, token usage, tool calls)
        let (messages, perf_metrics) = collect_baseline_metrics(
            agent,
            "Create a Wikipedia-style web page about Danni (Subfracture's AI agent) in a new index.html file. The page should be a complete, well-structured HTML document with proper head and body sections. Use heading tags (h1, h2, h3) to organize the content into clear sections. Include comprehensive information about Danni organized in a way similar to how Wikipedia presents technical topics. Remember to use your tools if applicable.".to_string()
        ).await;
