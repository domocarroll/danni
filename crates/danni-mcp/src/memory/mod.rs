...
impl MemoryServer {
    pub fn new() -> Self {
        let instructions = formatdoc! {r#"
             ...
             3. Upon agreement:
                ...
                - Confirm the desired storage location:
                  - Local storage (.danni/memory) for project-specific details.
                  - Global storage (~/.config/danni/memory) for user-wide data.
        ...
        // Check for .danni/memory in current directory
        let local_memory_dir = std::env::var("DANNI_WORKING_DIR")
            .map(PathBuf::from)
