    ...
    fn get_info(&self) -> ServerInfo {
        ...
        ServerInfo {
            server_info: Implementation {
                name: "danni-developer".to_string(),
...
impl DeveloperServer {
    ...
    pub async fn text_editor(
        ...
        if self.is_ignored(&path) {
            return Err(ErrorData::new(
                ErrorCode::INTERNAL_ERROR,
                format!(
                    "Access to '{}' is restricted by .danniignore",
                    path.display()
                ),
                None,
            ));
        ...
        for arg in &cmd_parts[1..] {
            ...
            if self.is_ignored(path) {
                return Err(ErrorData::new(
                    ErrorCode::INTERNAL_ERROR,
                    format!(
                        "The command attempts to access '{}' which is restricted by .danniignore",
                        arg
                    ),
                    None,
                ));
    ...
    fn build_ignore_patterns(cwd: &PathBuf) -> Gitignore {
        ...
        let local_ignore_path = cwd.join(".danniignore");
...
mod tests {
    ...
    #[tokio::test]
    #[serial]
    async fn test_danni_ignore_basic_patterns() {
        let temp_dir = tempfile::tempdir().unwrap();
        std::env::set_current_dir(&temp_dir).unwrap();

        // Create .danniignore file with patterns
        fs::write(".danniignore", "secret.txt\n*.env").unwrap();
    ...
    async fn test_text_editor_respects_ignore_patterns() {
        ...
        // Create .danniignore file
        fs::write(".danniignore", "secret.txt").unwrap();
