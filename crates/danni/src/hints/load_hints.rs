...
pub const DANNI_HINTS_FILENAME: &str = ".dannihints";
...
pub fn load_hint_files(
    ...
    if !global_hints_contents.is_empty() {
        hints.push_str("\n### Global Hints\nThese are my global danni hints.\n");
...
mod tests {
    ...
    #[test]
    fn test_dannihints_when_present() {
        let dir = TempDir::new().unwrap();

        fs::write(dir.path().join(DANNI_HINTS_FILENAME), "Test hint content").unwrap();
        let gitignore = create_dummy_gitignore();
        let hints = load_hint_files(dir.path(), &[DANNI_HINTS_FILENAME.to_string()], &gitignore);

        assert!(hints.contains("Test hint content"));
    }

    #[test]
    fn test_dannihints_when_missing() {
        let dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_dannihints_multiple_filenames() {
        let dir = TempDir::new().unwrap();
        ...
        .unwrap();
        fs::write(
            dir.path().join(DANNI_HINTS_FILENAME),
            "Custom hints file content from .dannihints",
        )
        .unwrap();
        ...
        let hints = load_hint_files(
            dir.path(),
            &["CLAUDE.md".to_string(), DANNI_HINTS_FILENAME.to_string()],
            &gitignore,
        );

        assert!(hints.contains("Custom hints file content from CLAUDE.md"));
        assert!(hints.contains("Custom hints file content from .dannihints"));
    }

    #[test]
    fn test_dannihints_configurable_filename() {
        let dir = TempDir::new().unwrap();
        ...
        assert!(!hints.contains(".dannihints")); // Make sure it's not loading the default
    }
    ...
    fn test_nested_goosehints_with_git_root() {
        ...
        fs::write(
            project_root.join(DANNI_HINTS_FILENAME),
            "Root hints content",
        )
        .unwrap();
        ...
        fs::write(subdir.join(DANNI_HINTS_FILENAME), "Subdir hints content").unwrap();
        let current_dir = subdir.join("current_dir");
        fs::create_dir(&current_dir).unwrap();
        fs::write(
            current_dir.join(DANNI_HINTS_FILENAME),
        ...
        let hints = load_hint_files(
            &current_dir,
            &[DANNI_HINTS_FILENAME.to_string()],
            &gitignore,
        );
    ...
    fn test_nested_goosehints_without_git_root() {
        ...
        fs::write(base_dir.join(DANNI_HINTS_FILENAME), "Base hints content").unwrap();
    ...
    fn test_nested_goosehints_mixed_filenames() {
        ...
        fs::create_dir(&subdir).unwrap();
        fs::write(
            subdir.join(DANNI_HINTS_FILENAME),
            "Subdir .dannihints content",
        )
        .unwrap();
        ...
        assert!(hints.contains("Subdir .dannihints content"));
    }
...
Additional instructions here."#;
        fs::write(project_root.join(DANNI_HINTS_FILENAME), hints_content).unwrap();
...
Root level instructions"#;
        fs::write(project_root.join(DANNI_HINTS_FILENAME), root_hints_content).unwrap();
...
End of nested hints"#;
        fs::write(
            components_dir.join(DANNI_HINTS_FILENAME),
...
@../parent.md
End of hints"#;
        fs::write(current_dir.join(DANNI_HINTS_FILENAME), hints_content).unwrap();
...
@../root_file.md
End of hints"#;
        fs::write(subdir.join(DANNI_HINTS_FILENAME), hints_content).unwrap();
        let gitignore = create_dummy_gitignore();

        let hints = load_hint_files(&subdir, &[DANNI_HINTS_FILENAME.to_string()], &gitignore);
