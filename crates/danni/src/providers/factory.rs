...
pub async fn create(name: &str, model: ModelConfig) -> Result<Arc<dyn Provider>> {
    ...
    if let Ok(lead_model_name) = config.get_param::<String>("DANNI_LEAD_MODEL") {
...
fn create_worker_model_config(default_model: &ModelConfig) -> Result<ModelConfig> {
    ...
    if let Ok(limit_str) = global_config.get_param::<String>("DANNI_WORKER_CONTEXT_LIMIT") {
        if let Ok(limit) = limit_str.parse::<usize>() {
            worker_config = worker_config.with_context_limit(Some(limit));
        }
    } else if let Ok(limit_str) = global_config.get_param::<String>("DANNI_CONTEXT_LIMIT") {
        if let Ok(limit) = limit_str.parse::<usize>() {
            worker_config = worker_config.with_context_limit(Some(limit));
        }
    }
...
mod tests {
    ...
    async fn test_create_lead_worker_provider() {
        ...
        _guard.set("DANNI_LEAD_MODEL", "gpt-4o");
        ...
        _guard.set("DANNI_LEAD_PROVIDER", "anthropic");
        _guard.set("DANNI_LEAD_TURNS", "5");
    ...
    async fn test_lead_model_env_vars_with_defaults() {
        let _guard = EnvVarGuard::new(&[
            ...
            "DANNI_LEAD_PROVIDER",
            "DANNI_LEAD_TURNS",
            "DANNI_LEAD_FAILURE_THRESHOLD",
            "DANNI_LEAD_FALLBACK_TURNS",
        ]);

        _guard.set("DANNI_LEAD_MODEL", "grok-3");
        ...
        _guard.set("DANNI_LEAD_TURNS", "7");
        _guard.set("DANNI_LEAD_FAILURE_THRESHOLD", "4");
        _guard.set("DANNI_LEAD_FALLBACK_TURNS", "3");
    ...
    fn test_worker_model_preserves_original_context_limit() {
        ...
        _guard.set("DANNI_WORKER_CONTEXT_LIMIT", "32000");
        let _result = create_lead_worker_from_env("openai", &default_model, "gpt-4o");

        _guard.set("DANNI_CONTEXT_LIMIT", "64000");
        let _result = create_lead_worker_from_env("openai", &default_model, "gpt-4o");
    }
}
