...
use crate::config::{DanniMode, PermissionManager};
...
impl PermissionInspector {
    ...
    pub async fn update_mode(&self, new_mode: DanniMode) {
        ...
        for request in tool_requests {
            if let Ok(tool_call) = &request.tool_call {
                ...
                let action = match *mode {
                    DanniMode::Chat => continue,
                    DanniMode::Auto => InspectionAction::Allow,
                    DanniMode::Approve | DanniMode::SmartApprove => {
