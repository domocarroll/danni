use danni::agents::recipe_tools::dynamic_task_tools::{
...
mod tests {
    ...
    async fn test_create_multiple_tasks() {
        use danni::agents::subagent_execution_tool::tasks_manager::TasksManager;
    ...
    fn test_with_settings() {
        ...
        assert_eq!(settings.danni_provider, Some("openai".to_string()));
        assert_eq!(settings.danni_model, Some("gpt-4".to_string()));
    ...
    fn test_extension_shortnames() {
        ...
        if !extensions.is_empty() {
            ...
            assert!(matches!(
                &extensions[0],
                danni::agents::extension::ExtensionConfig::Builtin { .. }
                    | danni::agents::extension::ExtensionConfig::Stdio { .. }
                    | danni::agents::extension::ExtensionConfig::Sse { .. }
                    | danni::agents::extension::ExtensionConfig::StreamableHttp { .. }
                    | danni::agents::extension::ExtensionConfig::Frontend { .. }
                    | danni::agents::extension::ExtensionConfig::InlinePython { .. }
            ));
        }
    }
    ...
    fn test_mixed_extension_formats() {
        ...
        if let Some(last) = extensions.last() {
            match last {
                danni::agents::extension::ExtensionConfig::Stdio { name, .. } => {
    ...
    fn test_unknown_extension_shortname() {
        ...
        match &extensions[0] {
            danni::agents::extension::ExtensionConfig::Builtin { name, .. } => {
