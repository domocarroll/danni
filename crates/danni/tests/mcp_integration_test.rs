...
use danni::agents::extension_manager::ExtensionManager;
...
async fn test_replayed_session(
    ...
    std::env::set_var("DANNI_MCP_CLIENT_VERSION", "0.0.0");

    // Setup test file for developer extension tests
    let test_file_path = "/tmp/danni_test/danni.txt";
    if let Some(parent) = std::path::Path::new(test_file_path).parent() {
        fs::create_dir_all(parent).ok();
    }
    fs::write(test_file_path, "# danni\n").ok();
    ...
    let mode = if env::var("DANNI_RECORD_MCP").is_ok() {
        TestMode::Record
    } else {
