...
use danni::config::PermissionManager;

use danni::agents::ExtensionConfig;
use danni::config::{Config, DanniMode};
use danni::model::ModelConfig;
use danni::prompt_template::render_global_file;
use danni::providers::{create, create_with_named_model};
use danni::recipe::Recipe;
use danni::recipe_deeplink;
use danni::session::session_manager::SessionType;
use danni::session::{Session, SessionManager};
use danni::{
...
async fn resume_agent(
    ...
    if payload.load_model_and_extensions {
        ...
        let provider_result = async {
            let provider_name: String = config.get_danni_provider().map_err(|_| ErrorResponse {
                message: "Could not configure agent: missing provider".into(),
                status: StatusCode::INTERNAL_SERVER_ERROR,
            })?;

            let model: String = config.get_danni_model().map_err(|_| ErrorResponse {
    ...
    let mut tools: Vec<ToolInfo> = agent
        ...
        .map(|tool| {
            let permission = permission_manager
                ...
                .or_else(|| {
                    if danni_mode == DanniMode::SmartApprove {
                        permission_manager.get_smart_approve_permission(&tool.name)
                    } else if danni_mode == DanniMode::Approve {
...
async fn update_agent_provider(
    ...
    let config = Config::global();
    let model = match payload.model.or_else(|| config.get_danni_model().ok()) {
        Some(m) => m,
        None => {
