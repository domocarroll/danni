...
use danni::config::declarative_providers::LoadedProvider;
use danni::config::paths::Paths;
use danni::config::ExtensionEntry;
use danni::config::{Config, ConfigError};
use danni::model::ModelConfig;
use danni::providers::base::{ProviderMetadata, ProviderType};
use danni::providers::create_with_default_model;
use danni::providers::pricing::{
    get_all_pricing, get_model_pricing, parse_model_id, refresh_pricing,
};
use danni::providers::providers as get_providers;
use danni::{agents::ExtensionConfig, config::permission::PermissionLevel};
use http::StatusCode;
...
pub async fn get_extensions() -> Result<Json<ExtensionResponse>, StatusCode> {
    let extensions = danni::config::get_all_extensions();
...
pub async fn add_extension(
    ...
    let extensions = danni::config::get_all_extensions();
    let key = danni::config::extensions::name_to_key(&extension_query.name);

    let is_update = extensions.iter().any(|e| e.config.key() == key);

    danni::config::set_extension(ExtensionEntry {
...
pub async fn remove_extension(Path(name): Path<String>) -> Result<Json<String>, StatusCode> {
    let key = danni::config::extensions::name_to_key(&name);
    danni::config::remove_extension(&key);
...
pub async fn get_provider_models(
    ...
    let loaded_provider = danni::config::declarative_providers::load_provider(name.as_str()).ok();
    ...
    let provider = danni::providers::create(&name, model_config)
        .await
    ...
    match provider.fetch_supported_models().await {
        ...
        Err(provider_error) => {
            use danni::providers::errors::ProviderError;
...
pub async fn init_config() -> Result<Json<String>, StatusCode> {
    ...
    match danni::config::base::load_init_config_from_workspace() {
...
pub async fn upsert_permissions(
    ...
    let mut permission_manager = danni::config::PermissionManager::default();
...
pub async fn create_custom_provider(
    ...
    let config = danni::config::declarative_providers::create_custom_provider(
        &request.engine,
    ...
    if let Err(e) = danni::providers::refresh_custom_providers().await {
...
pub async fn get_custom_provider(
    ...
    let loaded_provider = danni::config::declarative_providers::load_provider(id.as_str())
...
pub async fn remove_custom_provider(Path(id): Path<String>) -> Result<Json<String>, StatusCode> {
    danni::config::declarative_providers::remove_custom_provider(&id)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn update_custom_provider(
    ...
    danni::config::declarative_providers::update_custom_provider(
        &id,
        &request.engine,
        request.display_name,
        request.api_url,
...
pub async fn set_config_provider(
    ...
    create_with_default_model(&provider)
        ...
        .and_then(|_| {
            let config = Config::global();
            config
                .set_danni_provider(provider)
                .and_then(|_| config.set_danni_model(model))
...
mod tests {
    ...
    async fn test_read_model_limits() {
        ...
        let limits: Vec<danni::model::ModelLimitConfig> = serde_json::from_value(response).unwrap();
