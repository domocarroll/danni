...
pub fn extract_recipe_info_from_cli(
    ...
    let recipe_info = RecipeInfo {
        session_settings: recipe.settings.map(|s| SessionSettings {
            danni_provider: s.danni_provider,
            danni_model: s.danni_model,
...
mod tests {
    ...
    fn test_extract_recipe_info_from_cli_basic() {
        ...
        assert_eq!(settings.danni_provider, Some("test_provider".to_string()));
        assert_eq!(settings.danni_model, Some("test_model".to_string()));
...
settings:
  danni_provider: test_provider
  danni_model: test_model
  temperature: 0.7
sub_recipes:
