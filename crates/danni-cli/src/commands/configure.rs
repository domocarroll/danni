use crate::recipes::github_recipe::DANNI_RECIPE_GITHUB_REPO_CONFIG_KEY;
use cliclack::spinner;
use console::style;
use danni::agents::extension::ToolInfo;
use danni::agents::extension_manager::get_parameter_names;
use danni::agents::Agent;
use danni::agents::{extension::Envs, ExtensionConfig};
use danni::config::declarative_providers::{create_custom_provider, remove_custom_provider};
use danni::config::extensions::{
    get_all_extension_names, get_all_extensions, get_enabled_extensions, get_extension_by_name,
    name_to_key, remove_extension, set_extension, set_extension_enabled,
};
use danni::config::paths::Paths;
use danni::config::permission::PermissionLevel;
use danni::config::signup_tetrate::TetrateAuth;
use danni::config::{
    configure_tetrate, Config, ConfigError, ExperimentManager, ExtensionEntry, DanniMode,
    PermissionManager,
};
use danni::conversation::message::Message;
use danni::model::ModelConfig;
use danni::providers::provider_test::test_provider_configuration;
use danni::providers::{create, providers};
...
pub async fn handle_configure() -> anyhow::Result<()> {
    ...
    if !config.exists() {
        ...
        println!(
            "{}",
            style("Welcome to danni! Let's get you set up with a provider.").dim()
        );
        println!(
            "{}",
        ...
        println!();
        cliclack::intro(style(" danni-configure ").on_cyan().black())?;
        ...
        let action = cliclack::select("What would you like to configure?")
            ...
            .item(
                "settings",
                "danni settings",
                "Set the danni mode, Tool Output, Tool Permissions, Experiment, danni recipe github repo and more",
            )
            .interact()?;
...
fn select_model_from_list(
    models: &[String],
    provider_meta: &danni::providers::base::ProviderMetadata,
    ...
    match config.set_secret(key_name, &value) {
        ...
        Err(e) => {
            cliclack::outro(style(format!(
                "Failed to store {} securely: {}. Please ensure your system's secure storage is accessible. Alternatively you can run with DANNI_DISABLE_KEYRING=true or set the key in your environment variables",
                key_name, e
            )).on_red().white())?;
            Ok(false)
        }
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let current_provider: Option<String> = config.get_danni_provider().ok();
    ...
    let model: String = match models_res {
        ...
        Ok(None) => {
            let default_model =
                std::env::var("DANNI_MODEL").unwrap_or(provider_meta.default_model.clone());
    ...
    let toolshim_enabled = std::env::var("DANNI_TOOLSHIM")
        .map(|val| val == "1" || val.to_lowercase() == "true")
        .unwrap_or(false);
    let toolshim_model = std::env::var("DANNI_TOOLSHIM_OLLAMA_MODEL").ok();

    match test_provider_configuration(provider_name, &model, toolshim_enabled, toolshim_model).await
    {
        Ok(()) => {
            config.set_danni_provider(provider_name)?;
            config.set_danni_model(&model)?;
...
/// Configure extensions that can be used with danni
...
pub fn configure_extensions_dialog() -> anyhow::Result<()> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with danni",
        )
        .item(
            "stdio",
    ...
    match extension_type {
        // TODO we'll want a place to collect all these options, maybe just an enum in danni-mcp
        "built-in" => {
            let extensions = vec![
                (
                    "autovisualiser",
            ...
            let timeout: u64 = cliclack::input("Please set the timeout for this tool (in secs):")
                .placeholder(&danni::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
pub async fn configure_settings_dialog() -> anyhow::Result<()> {
    let setting_type = cliclack::select("What setting would you like to configure?")
        .item("danni_mode", "danni mode", "Configure danni mode")
        .item(
            "danni_router_strategy",
        ...
        .item(
            "recipe",
            "danni recipe github repo",
            "danni will pull recipes from this repo if not found locally.",
        )
        .interact()?;
    ...
    match setting_type {
        "danni_mode" => {
            configure_danni_mode_dialog()?;
        }
        "danni_router_strategy" => {
            configure_danni_router_strategy_dialog()?;
        }
        "tool_permission" => {
...
pub fn configure_danni_mode_dialog() -> anyhow::Result<()> {
    let config = Config::global();

    // Check if DANNI_MODE is set as an environment variable
    if std::env::var("DANNI_MODE").is_ok() {
        let _ = cliclack::log::info("Notice: DANNI_MODE environment variable is set and will override the configuration here.");
    }

    let mode = cliclack::select("Which danni mode would you like to configure?")
        .item(
            DanniMode::Auto,
            "Auto Mode",
    ...
    config.set_danni_mode(mode)?;
    let msg = match mode {
        DanniMode::Auto => "Set to Auto Mode - full file modification enabled",
        DanniMode::Approve => "Set to Approve Mode - all tools and modifications require approval",
        DanniMode::SmartApprove => "Set to Smart Approve Mode - modifications require approval",
        DanniMode::Chat => "Set to Chat Mode - no tools or modifications enabled",
    };
    cliclack::outro(msg)?;
    Ok(())
}

pub fn configure_danni_router_strategy_dialog() -> anyhow::Result<()> {
    let config = Config::global();
    ...
    config.set_param("DANNI_ENABLE_ROUTER", enable_router)?;
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    let config = Config::global();
    // Check if DANNI_CLI_MIN_PRIORITY is set as an environment variable
    if std::env::var("DANNI_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info("Notice: DANNI_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.");
    }
    ...
    match tool_log_level {
        "high" => {
            config.set_param("DANNI_CLI_MIN_PRIORITY", 0.8)?;
            cliclack::outro("Showing tool output of high importance only.")?;
        }
        "medium" => {
            config.set_param("DANNI_CLI_MIN_PRIORITY", 0.2)?;
            cliclack::outro("Showing tool output of medium importance.")?;
        }
        "all" => {
            config.set_param("DANNI_CLI_MIN_PRIORITY", 0.0)?;
...
/// Configure experiment features that can be used with danni
...
fn configure_recipe_dialog() -> anyhow::Result<()> {
    let key_name = DANNI_RECIPE_GITHUB_REPO_CONFIG_KEY;
    let config = Config::global();
    ...
    let mut recipe_repo_input = cliclack::input(
        "Enter your danni recipe Github repo (owner/repo): eg: my_org/danni-recipes",
    )
    .required(false);
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    let current_max_turns: u32 = config.get_param("DANNI_MAX_TURNS").unwrap_or(1000);
    ...
    config.set_param("DANNI_MAX_TURNS", max_turns)?;

    cliclack::outro(format!(
        "Set maximum turns to {} - danni will ask for input after {} consecutive actions",
        max_turns, max_turns
    ))?;
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use danni::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    use danni::conversation::message::Message;
    use danni::providers::create;
    ...
    let configured_model: String = config.get_danni_model()?;
    let model_config = match danni::model::ModelConfig::new(&configured_model) {
        Ok(config) => config,
        Err(e) => {
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Builtin {
                                ...
                                display_name: Some(danni::config::DEFAULT_DISPLAY_NAME.to_string()),
                                timeout: Some(danni::config::DEFAULT_EXTENSION_TIMEOUT),
                                bundled: Some(true),
...
fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = danni::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        danni::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
