use anyhow::Result;
use console::style;
use danni::recipe::validate_recipe::validate_recipe_template_from_file;
    ...
    match generate_deeplink(recipe_name) {
        Ok((deeplink_url, recipe)) => {
            ...
            match open::that(&deeplink_url) {
                Ok(_) => {
                    println!(
                        "{} Opened recipe '{}' in Danni Desktop",
                        style("✓").green().bold(),
                        recipe.title
                    );
                    Ok(())
                ...
                Err(err) => {
                    println!(
                        "{} Failed to open recipe in Danni Desktop: {}",
                        style("✗").red().bold(),
                        err
                    );
                    println!("Generated deeplink: {}", deeplink_url);
                    println!("You can manually copy and open the URL above, or ensure Danni Desktop is installed.");
...
fn generate_deeplink(recipe_name: &str) -> Result<(String, danni::recipe::Recipe)> {
    let recipe_file = load_recipe_file(recipe_name)?;
