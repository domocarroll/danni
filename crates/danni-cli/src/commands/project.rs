...
pub fn handle_project_default() -> Result<()> {
    ...
    if projects.is_empty() {
        ...
        let mut command = std::process::Command::new("danni");
        command.arg("session");
        let status = command.status()?;

        if !status.success() {
            println!("Failed to run danni. Exit code: {:?}", status.code());
        }
        return Ok(());
    }
    ...
    let _ = intro("danni Project Manager");
    ...
    match choice {
        "resume" => {
            ...
            // Build the command to run danni
            let mut command = std::process::Command::new("danni");
            command.arg("session");
        ...
        "fresh" => {
            ...
            // Build the command to run danni with a fresh session
            let mut command = std::process::Command::new("danni");
            command.arg("session");
