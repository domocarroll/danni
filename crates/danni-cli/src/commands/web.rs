...
use danni::agents::{Agent, AgentEvent};
use danni::conversation::message::Message as DanniMessage;
...
async fn auth_middleware(
    ...
    *response.status_mut() = StatusCode::UNAUTHORIZED;
    response.headers_mut().insert(
        "WWW-Authenticate",
        "Basic realm=\"Danni Web Interface\"".parse().unwrap(),
    );
    Ok(response)
}
...
pub async fn handle_web(
    ...
    // Setup logging
    crate::logging::setup_logging(Some("danni-web"), None)?;

    let config = danni::config::Config::global();

    let provider_name: String = match config.get_danni_provider() {
        Ok(p) => p,
        Err(_) => {
            eprintln!("No provider configured. Run 'danni configure' first");
            std::process::exit(1);
        }
    };

    let model: String = match config.get_danni_model() {
        Ok(m) => m,
        Err(_) => {
            eprintln!("No model configured. Run 'danni configure' first");
            std::process::exit(1);
        }
    };

    let model_config = danni::model::ModelConfig::new(&model)?;

    // Create the agent
    let agent = Agent::new();
    let provider = danni::providers::create(&provider_name, model_config).await?;
    agent.update_provider(provider).await?;

    // Load and enable extensions from config
    let enabled_configs = danni::config::get_enabled_extensions();
...
async fn serve_session(
    ...
    let html_with_session = html.replace(
        ...
        &format!(
            "<script>window.DANNI_SESSION_NAME = '{}';</script>\n    <script src=\"/static/script.js\"></script>",
            session_name
        )
    );
...
async fn process_message_streaming(
    ...
    use danni::agents::SessionConfig;
    use danni::conversation::message::MessageContent;

    let user_message = DanniMessage::user().with_text(content.clone());

    let provider = agent.provider().await;
    if provider.is_err() {
        let error_msg = "I'm not properly configured yet. Please configure a provider through the CLI first using `danni configure`.".to_string();
        let mut sender = sender.lock().await;
        let _ = sender
            .send(Message::Text(
                            ...
                            match content {
                                ...
                                MessageContent::ToolConfirmationRequest(confirmation) => {
                                    ...
                                    agent.handle_confirmation(
                                        ...
                                        danni::permission::PermissionConfirmation {
                                            principal_type: danni::permission::permission_confirmation::PrincipalType::Tool,
                                            permission: danni::permission::Permission::AllowOnce,
                                        }
                                    ).await;
                                }
