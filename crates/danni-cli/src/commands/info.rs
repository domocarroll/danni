...
use danni::session::session_manager::{DB_NAME, SESSIONS_FOLDER};
use serde_yaml;
...
use danni::config::base::CONFIG_YAML_NAME;
use std::fs;
use std::path::Path;
...
pub fn handle_info(verbose: bool) -> Result<()> {
    ...
    println!("{}", style("danni Version:").cyan().bold());
    ...
    if verbose {
        println!("\n{}", style("danni Configuration:").cyan().bold());
        let values = config.all_values()?;
        if values.is_empty() {
            println!("  No configuration values set");
            println!(
                "  Run '{}' to configure danni",
