...
use anyhow::Result;
use danni::agents::{Agent, SessionConfig};
use danni::config::{get_all_extensions, Config};
use danni::conversation::message::{Message, MessageContent};
use danni::conversation::Conversation;
use danni::mcp_utils::ToolResult;
...
fn extract_tool_locations(
    tool_request: &danni::conversation::message::ToolRequest,
    tool_response: &danni::conversation::message::ToolResponse,
...
impl DanniAcpAgent {
    async fn new(
        ...
        let model_config = danni::model::ModelConfig {
        ...
        for block in prompt {
            match block {
                ...
                acp::ContentBlock::Image(image) => {
                    // Danni supports images via base64 encoded data
...
impl acp::Agent for DanniAcpAgent {
    async fn initialize(
        &self,
    ...
    async fn new_session(
        ...
        let danni_session = SessionManager::create_session(
        ...
        sessions.insert(danni_session.id.clone(), session);

        info!("Created new ACP/danni session {}", danni_session.id);

        Ok(acp::NewSessionResponse {
            session_id: acp::SessionId(danni_session.id.into()),
            modes: None,
            meta: None,
        })
    }
    ...
    async fn load_session(
        ...
        // For now, will start a new session. We could use danni session storage as an enhancement
        // we would need to map ACP session IDs to danni session ids (which by default are auto generated)
        // normal danni session restore in CLI doesn't load conversation visually.
        //
        // Example flow:
        // - Load session file by session_id (might need to map ACP session IDs to Danni session paths)
    ...
    async fn prompt(&self, args: acp::PromptRequest) -> Result<acp::PromptResponse, acp::Error> {
        ...
        while let Some(event) = stream.next().await {
            ...
            match event {
                Ok(danni::agents::AgentEvent::Message(message)) => {
                    let mut sessions = self.sessions.lock().await;
                    let session = sessions
                        .get_mut(&session_id)
                        .ok_or_else(acp::Error::invalid_params)?;
...
pub async fn run_acp_agent() -> Result<()> {
    info!("Starting Danni ACP agent server on stdio");
    eprintln!("Danni ACP agent started. Listening on stdio...");
    ...
    local_set
        .run_until(async move {
            ...
            // Start up the DanniAcpAgent connected to stdio.
            let agent = DanniAcpAgent::new(tx)
                .await
