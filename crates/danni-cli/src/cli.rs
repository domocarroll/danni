...
use danni::config::{Config, ExtensionConfig};
use danni_mcp::mcp_server_runner::{serve, McpCommand};
use danni_mcp::{
...
use danni_bench::bench_config::BenchRunConfig;
use danni_bench::runners::bench_runner::BenchRunner;
use danni_bench::runners::eval_runner::EvalRunner;
use danni_bench::runners::metric_aggregator::MetricAggregator;
use danni_bench::runners::model_runner::ModelRunner;
use std::io::Read;
...
enum RecipeCommand {
    ...
    /// Open a recipe in Danni Desktop
    #[command(about = "Open a recipe in Danni Desktop")]
    Open {
...
#[derive(Subcommand)]
enum Command {
    /// Configure danni settings
    #[command(about = "Configure danni settings")]
    Configure {},

    /// Display danni configuration information
    #[command(about = "Display danni information")]
    Info {
    ...
    #[command(about = "Run one of the mcp servers bundled with danni")]
    Mcp {
        #[arg(value_parser = clap::value_parser!(McpCommand))]
        server: McpCommand,
    },

    /// Run danni as an ACP (Agent Client Protocol) agent
    #[command(about = "Run danni as an ACP agent server on stdio")]
    Acp {},
    ...
    Session {
        ...
        #[arg(
            ...
            long_help = "Add one or more builtin extensions that are bundled with danni by specifying their names, comma-separated",
            value_delimiter = ','
        )]
        builtins: Vec<String>,

        /// Enable TUI mode for Go TUI integration
        #[arg(
            long = "tui-mode",
            help = "Enable TUI mode with JSON event streaming",
            long_help = "Run in TUI mode, outputting JSON events to stdout for consumption by the Go TUI. This mode is used internally by the danni-tui application.",
            hide = true
        )]
        tui_mode: bool,
    },
    ...
    Run {
        ...
        #[arg(
            ...
            long = "text",
            value_name = "TEXT",
            help = "Input text to provide to danni directly",
            long_help = "Input text containing commands for danni. Use this in lieu of the instructions argument.",
        ...
        #[arg(
            ...
            help = "Dynamic parameters (e.g., --params username=alice --params channel_name=danni-channel)",
        ...
        #[arg(
            ...
            long_help = "Override the DANNI_PROVIDER environment variable for this run. Available providers include openai, anthropic, ollama, databricks, gemini-cli, claude-code, and others."
        )]
        ...
        #[arg(
            ...
            long_help = "Override the DANNI_MODEL environment variable for this run. The model must be supported by the specified provider."
        )]
        model: Option<String>,
    },
    ...
    /// Update the danni CLI version
    #[command(about = "Update the danni CLI version")]
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the danni CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,

        /// Enforce to re-configure danni during update
        #[arg(short, long, help = "Enforce to re-configure danni during update")]
        reconfigure: bool,
    },
...
pub struct RecipeInfo {
    ...
    pub sub_recipes: Option<Vec<danni::recipe::SubRecipe>>,
    pub final_output_response: Option<danni::recipe::Response>,
    pub retry_config: Option<danni::agents::types::RetryConfig>,
}
        ...
        Some(Command::Session {
            ...
            return match command {
                ...
                None => {
                    ...
                    tracing::info!(
                        counter.danni.session_starts = 1,
                        session_type,
                        interactive = true,
                        "Session started"
                    );
                    ...
                    tracing::info!(
                        counter.danni.session_completions = 1,
                        session_type,
                        exit_type,
                    ...
                    tracing::info!(
                        counter.danni.session_duration_ms = session_duration.as_millis() as u64,
                        session_type,
                        "Session duration"
                    );
                    ...
                    if total_tokens > 0 {
                        tracing::info!(
                            counter.danni.session_tokens = total_tokens,
                            session_type,
                            "Session tokens"
                        );
                    }
            ...
            let (input_config, recipe_info) = match (instructions, input_text, recipe) {
                ...
                (Some(file), _, _) => {
                    let contents = std::fs::read_to_string(&file).unwrap_or_else(|err| {
                        eprintln!(
                            "Instruction file not found — did you mean to use danni run --text?\n{}",
                            err
                        );
                        std::process::exit(1);
                    });
                ...
                (_, _, Some(recipe_name)) => {
                    ...
                    let recipe_version =
                        crate::recipes::search_recipe::load_recipe_file(&recipe_name)
                            .ok()
                            .and_then(|rf| {
                                danni::recipe::template_recipe::parse_recipe_content(
                                    &rf.content,
