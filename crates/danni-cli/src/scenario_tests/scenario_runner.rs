...
use danni::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("DANNI_TEST_PROVIDER") {
...
{
    use danni::config::ExtensionConfig;
    use tokio::sync::Mutex;

    danni::agents::moim::SKIP.with(|f| f.set(true));
    ...
    agent
        .update_provider(provider_arc as Arc<dyn danni::providers::base::Provider>)
        .await?;
