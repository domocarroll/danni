use super::completion::DanniCompleter;
use anyhow::Result;
...
pub fn get_input(
    editor: &mut Editor<DanniCompleter, rustyline::history::DefaultHistory>,
) -> Result<InputResult> {
...
fn handle_slash_command(input: &str) -> Option<InputResult> {
    ...
    match input {
        ...
        s if s.starts_with(CMD_MODE) => Some(InputResult::DanniMode(
...
/// Returns a styled prompt with the danni face "( O)>" followed by a space.
...
/mode <name> - Set the danni mode to use ('auto', 'approve', 'chat', 'smart_approve')
/plan <message_text> -  Enters 'plan' mode with optional message. Create a plan based on the current messages and asks user if they want to act on it.
                        If user acts on the plan, danni mode is set to 'auto' and returns to 'normal' danni mode.
                        To warm up danni before using '/plan', we recommend setting '/mode approve' & putting appropriate context into danni.
                        The model is used based on $DANNI_PLANNER_PROVIDER and $DANNI_PLANNER_MODEL environment variables.
                        If no model is set, the default model is used.
/endplan - Exit plan mode and return to 'normal' danni mode.
...
mod tests {
    ...
    fn test_get_input_prompt_string() {
        ...
        // Prompt should contain the danni face
