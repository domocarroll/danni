...
use console::style;
use danni::agents::types::{RetryConfig, SessionConfig};
...
use danni::recipe::{Response, SubRecipe};

use danni::agents::extension::PlatformExtensionContext;
use danni::session::session_manager::SessionType;
use danni::session::SessionManager;
use danni::session::{EnabledExtensionsState, ExtensionState};
...
/// Configuration for building a new Danni session
///
...
pub struct SessionBuilderConfig {
    ...
    /// Enable TUI mode with JSON event streaming
    pub tui_mode: bool,
}
...
async fn offer_extension_debugging_help(
    extension_name: &str,
    error_message: &str,
    provider: Arc<dyn danni::providers::base::Provider>,
    interactive: bool,
) -> Result<(), anyhow::Error> {
    ...
    let mut debug_session = CliSession::new(
        ...
        None,
        "text".to_string(),
        false, // Debug sessions don't use TUI mode
    )
    .await;
...
pub struct SessionSettings {
    pub danni_model: Option<String>,
    pub danni_provider: Option<String>,
...
pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    ...
    let provider_name = session_config
        ...
        .or_else(|| {
            session_config
                .settings
                .as_ref()
                .and_then(|s| s.danni_provider.clone())
        })
        .or_else(|| config.get_danni_provider().ok())
    ...
    let model_name = session_config
        ...
        .or_else(|| {
            session_config
                .settings
                .as_ref()
                .and_then(|s| s.danni_model.clone())
        })
        .or_else(|| config.get_danni_model().ok())
    ...
    let model_config = danni::model::ModelConfig::new(&model_name)
        .unwrap_or_else(|e| {
    ...
    let new_provider = match create(&provider_name, model_config).await {
        ...
        Err(e) => {
            output::render_error(&format!(
                "Error {}.\n\
                Please check your system keychain and run 'danni configure' again.\n\
    ...
    let debug_mode = session_config.debug || config.get_param("DANNI_DEBUG").unwrap_or(false);
    ...
    let system_prompt_file: Option<String> = config.get_param("DANNI_SYSTEM_PROMPT_FILE_PATH").ok();
