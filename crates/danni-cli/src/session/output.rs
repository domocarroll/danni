...
use danni::config::Config;
use danni::conversation::message::{Message, MessageContent, ToolRequest, ToolResponse};
use danni::providers::pricing::get_model_pricing;
use danni::providers::pricing::parse_model_id;
...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("DANNI_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("DANNI_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("DANNI_CLI_THEME", theme.as_config_string())
    ...
    if let Err(e) = config.set_param("DANNI_CLI_THEME", theme_str) {
    ...
    for content in &message.content {
        match content {
            ...
            MessageContent::Thinking(thinking) => {
                if std::env::var("DANNI_CLI_SHOW_THINKING").is_ok()
            ...
            MessageContent::SystemNotification(notification) => {
                use danni::conversation::message::SystemNotificationType;
...
pub fn danni_mode_message(text: &str) {
    println!("\n{}", style(text).yellow(),);
}
...
pub fn display_session_info(
    ...
    provider_instance: Option<&Arc<dyn danni::providers::base::Provider>>,
) {
...
pub fn display_greeting() {
    eprintln!("\ndanni is running! Enter your instructions, or try asking what danni can do.\n");
}
...
impl McpSpinners {
    pub fn new() -> Self {
        // Use hidden MultiProgress to avoid TTY issues
        let multi_bar = MultiProgress::new();
        multi_bar.set_draw_target(indicatif::ProgressDrawTarget::hidden());
