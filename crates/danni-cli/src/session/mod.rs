...
mod thinking;
mod tui_events;
mod tui_event_bridge;
mod tui_interactive;

use crate::session::task_execution_display::{
    format_task_execution_notification, TASK_EXECUTION_NOTIFICATION_TYPE,
};
use danni::conversation::Conversation;
use std::io::Write;
use std::str::FromStr;
...
use danni::permission::permission_confirmation::PrincipalType;
use danni::permission::Permission;
use danni::permission::PermissionConfirmation;
use danni::providers::base::Provider;
use danni::utils::safe_truncate;

use anyhow::{Context, Result};
use completion::DanniCompleter;
use danni::agents::extension::{Envs, ExtensionConfig, PLATFORM_EXTENSIONS};
use danni::agents::types::RetryConfig;
use danni::agents::{Agent, SessionConfig, MANUAL_COMPACT_TRIGGER};
use danni::config::{Config, DanniMode};
use danni::providers::pricing::initialize_pricing_cache;
...
pub struct CliSession {
    ...
    tui_mode: bool,
    tui_emitter: tui_events::TuiEventEmitter,
}
...
impl CliSession {
    ...
    pub async fn new(
        ...
        let tui_emitter = tui_events::TuiEventEmitter::new(tui_mode);
    ...
    pub async fn add_extension(&mut self, extension_command: String) -> Result<()> {
        ...
        let config = ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: danni::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
    ...
    pub async fn interactive(&mut self, prompt: Option<String>) -> Result<()> {
        // If TUI mode is enabled, use the TUI interactive handler
        if self.tui_mode {
            return self.interactive_tui(prompt).await;
        }
        ...
        let mut editor =
            rustyline::Editor::<DanniCompleter, rustyline::history::DefaultHistory>::with_config(
                config,
            )?;

        // Set up the completer with a reference to the completion cache
        let completer = DanniCompleter::new(self.completion_cache.clone());
        ...
        let save_history =
            |editor: &mut rustyline::Editor<DanniCompleter, rustyline::history::DefaultHistory>| {
        ...
        loop {
            ...
            match input::get_input(&mut editor)? {
                ...
                input::InputResult::DanniMode(mode) => {
                    save_history(&mut editor);

                    let config = Config::global();
                    let mode = match DanniMode::from_str(&mode.to_lowercase()) {
                        Ok(mode) => mode,
                        Err(_) => {
                    ...
                    output::danni_mode_message(&format!("Danni mode set to '{:?}'", mode));
                    continue;
                }
        ...
        match planner_response_type {
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    self.run_mode = RunMode::Normal;
                    // set danni mode: auto if that isn't already the case
                    let config = Config::global();
                    let curr_danni_mode = config.get_danni_mode().unwrap_or(DanniMode::Auto);
                    if curr_danni_mode != DanniMode::Auto {
                        config.set_danni_mode(DanniMode::Auto).unwrap();
                    }
                    ...
                    // Reset run & danni mode
                    if curr_danni_mode != DanniMode::Auto {
                        config.set_danni_mode(curr_danni_mode)?;
                    }
                } else {
    ...
    async fn process_agent_response(
        ...
        // Only set up Ctrl+C handler if not in TUI mode (TUI handles its own signals)
        let _drop_handle = if !self.tui_mode {
                ...
                result = stream.next() => {
                    match result {
                        Some(Ok(AgentEvent::Message(message))) => {
                            // Emit TUI event if in TUI mode
                            let _ = self.emit_agent_event(&AgentEvent::Message(message.clone()));
                            ...
                            if let Some(MessageContent::ToolConfirmationRequest(confirmation)) = message.content.first() {
                                // In TUI mode, auto-allow tool calls (no interactive prompts)
                                let permission = if self.tui_mode {
                                    ...
                                    let prompt = if let Some(security_message) = &confirmation.prompt {
                                        ...
                                        "Danni would like to call the above tool, do you allow?".to_string()
                                    };
                                ...
                                for content in &message.content {
                                    if let MessageContent::ToolRequest(tool_request) = content {
                                        if let Ok(tool_call) = &tool_request.tool_call {
                                            tracing::info!(counter.danni.tool_calls = 1,
                                    ...
                                    if let MessageContent::ToolResponse(tool_response) = content {
                                        ...
                                        tracing::info!(
                                            counter.danni.tool_completions = 1,
                                            tool_name = %tool_name,
                                ...
                                if interactive && !self.tui_mode {output::hide_thinking()};
                                let _ = progress_bars.hide();

                                // Don't render in JSON mode or TUI mode
                                if !is_json_mode && !self.tui_mode {
                                                ...
                                                let formatted = match notification_type {
                                                    ...
                                                    Some("response_generated") => {
                                                        ...
                                                        let min_priority = config
                                                            .get_param::<f32>("DANNI_CLI_MIN_PRIORITY")
                                                            .ok()
                                                            .unwrap_or(0.5);
                        ...
                        Some(Err(e)) => {
                            ...
                            if e.downcast_ref::<danni::providers::errors::ProviderError>()
                                .map(|provider_error| matches!(provider_error, danni::providers::errors::ProviderError::ContextLengthExceeded(_)))
                                .unwrap_or(false) {
    ...
    pub async fn get_session(&self) -> Result<danni::session::Session> {
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let show_cost = config
            .get_param::<bool>("DANNI_CLI_SHOW_COST")
            .unwrap_or(false);
...
async fn get_reasoner() -> Result<Arc<dyn Provider>, anyhow::Error> {
    ...
    let provider = if let Ok(provider) = config.get_param::<String>("DANNI_PLANNER_PROVIDER") {
        provider
    } else {
        println!("WARNING: DANNI_PLANNER_PROVIDER not found. Using default provider...");
        config
            .get_danni_provider()
            .expect("No provider configured. Run 'danni configure' first")
    };

    // Try planner-specific model first, fallback to default model
    let model = if let Ok(model) = config.get_param::<String>("DANNI_PLANNER_MODEL") {
        model
    } else {
        println!("WARNING: DANNI_PLANNER_MODEL not found. Using default model...");
        config
            .get_danni_model()
            .expect("No model configured. Run 'danni configure' first")
    };

    let model_config =
        ModelConfig::new_with_context_env(model, Some("DANNI_PLANNER_CONTEXT_LIMIT"))?;
