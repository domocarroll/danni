...
If you use Danni, Copilot, Claude, or other AI tools to help with your PRs:  
...
danni includes rust binaries alongside an electron app for the GUI. To work
...
First let's compile danni and try it out

```
cargo build
```

when that is done, you should now have debug builds of the binaries like the danni cli:

```
./target/debug/danni --help
```

If you haven't used the CLI before, you can use this compiled version to do first time configuration:

```
./target/debug/danni configure
```

And then once you have a connection to an LLM provider working, you can run a session!

```
./target/debug/danni session
```

These same commands can be recompiled and immediately run using `cargo run -p danni-cli` for iteration.
...
you can talk to danni!

You can now make changes in the code in ui/desktop to iterate on the GUI half of danni.

### Regenerating the OpenAPI schema

The file `ui/desktop/openapi.json` is automatically generated during the build.
It is written by the `generate_schema` binary in `crates/danni-server`.
...
Changes to the API should be made in the Rust source under `crates/danni-server/src/`.

### Debugging

To debug the Danni server, you can run it from your preferred IDE. How to configure the command
to start the server will depend on your IDE. The command to run is:

```
export GOOSE_SERVER__SECRET_KEY=test
cargo run --package danni-server --bin dannid -- agent   # or: `just run-server`
```
...
1. Go to https://github.com/block/danni and click “Fork” (top-right corner).
2. This creates https://github.com/<your-username>/danni under your GitHub account.
3. Clone your fork (not the main repo):

```
git clone https://github.com/<your-username>/danni.git
cd danni
```
...
```
git remote add upstream https://github.com/block/danni.git
```
...
8. Open a Pull Request from your branch on your fork to block/danni’s main branch.
...
You can change the provider danni points to via the `GOOSE_PROVIDER` env var. If you already
...
When testing changes or running multiple danni configurations, use `GOOSE_PATH_ROOT` to isolate your data:

```bash
# Test with a clean environment
export GOOSE_PATH_ROOT="/tmp/danni-test"
./target/debug/danni session

# Or for a single command
GOOSE_PATH_ROOT="/tmp/danni-dev" cargo run -p danni-cli -- session
```

This creates isolated `config/`, `data/`, and `state/` directories under the specified path, preventing your test sessions from affecting your main danni installation. See the [environment variables guide](./documentation/docs/guides/environment-variables.md#development--testing) for more details.

## Enable traces in danni with [locally hosted Langfuse](https://langfuse.com/docs/deployment/self-host)
...
- Set the environment variables so that danni can connect to the langfuse server:
...
[issues]: https://github.com/block/danni/issues
...
There are numerous ways to be an open source contributor and contribute to danni. We're here to help you on your way! Here are some suggestions to get started. If you have any questions or need help, feel free to reach out to us on [Discord](https://discord.gg/danni-oss).

- **Stars on GitHub:** If you resonate with our project and find it valuable, consider starring our danni on GitHub! 🌟
- **Ask Questions:** Your questions not only help us improve but also benefit the community. If you have a question, don't hesitate to ask it on [Discord](https://discord.gg/danni-oss).
- **Give Feedback:** Have a feature you want to see or encounter an issue with danni, [click here to open an issue](https://github.com/block/danni/issues/new/choose), [start a discussion](https://github.com/block/danni/discussions) or tell us on Discord.
- **Participate in Community Events:** We host a variety of community events and livestreams on Discord every month, ranging from workshops to brainstorming sessions. You can subscribe to our [events calendar](https://calget.com/c/t7jszrie) or follow us on [social media](https://linktr.ee/danni_oss) to stay in touch.
...
- **Spread the Word:** Help us reach more people by sharing danni's project, website, YouTube, and/or Twitter/X.
